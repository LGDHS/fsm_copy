...
func TestCancelBeforeGenericEvent(t *testing.T) {
	fsm := NewFSM(
		"start",
		Events{
			{Name: "run", Src: []string{"start","yu","sa","ds"}, Dst: "end"},
		},
		Callbacks{
			"before_event": func(e *Event) {
...
func ExampleNewFSM() {
	fsm := NewFSM(
		"green",
		Events{
			{Name: "warn", Src: []string{"green","yellow","ewa"}, Dst: "ye"},
	...
	fmt.Println("===============")
	fmt.Printf("Current:  %s",fsm.Current())
	fmt.Println()
	err := fsm.Event("warn")
	fmt.Println("======================")
	...
	fmt.Printf("after Current:  %s",fsm.Current())
	fmt.Println()
	// Output:
	// green
	// before_warn
